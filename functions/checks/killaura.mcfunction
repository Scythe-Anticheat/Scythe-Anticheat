...
execute @a[tag=killaura2,tag=!noaura,tag=!gliding,scores={leftclick=1..}] ~~~ execute @s[y=0,dy=256] ~~~ scoreboard players add @s killauravl 1
execute @a[tag=killaura2,tag=!noaura,tag=!gliding,scores={leftclick=1..}] ~~~ execute @s[y=0,dy=256] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" §1has failed §7(Combat) §4KillAura/A. VL= "},{"score":{"name":"@s","objective":"killauravl"}}]}
