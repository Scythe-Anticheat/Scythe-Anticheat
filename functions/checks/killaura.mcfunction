...
execute @s[tag=killaura2,tag=!noaura,tag=!gliding,tag=!dead,scores={leftclick=1..}] ~~~ scoreboard players add @s[y=0,dy=256,scores={aura_timer=10..}] killauravl 1
execute @s[tag=killaura2,tag=!noaura,tag=!gliding,tag=!dead,scores={leftclick=1..}] ~~~ execute @s[y=0,dy=256,scores={aura_timer=10..}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" §1has failed §7(Combat) §4KillAura/A. VL= "},{"score":{"name":"@s","objective":"killauravl"}}]}
