...
execute @s[type=player,tag=op] ~~~ scoreboard players set scythe:gamemode gmc 1
execute @s[type=player,tag=op] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has allowed gamemode 1 to be used!"}]}