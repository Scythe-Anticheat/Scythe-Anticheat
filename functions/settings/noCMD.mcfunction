...
execute @s[tag=!op] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" §rHas tried to toggle Force-CommandBlocksEnabled without perms!"}]}
...
execute @s[type=player,tag=op,scores={cmds=1}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has set CommandBlocksEnabled §4as disabled!"}]}
