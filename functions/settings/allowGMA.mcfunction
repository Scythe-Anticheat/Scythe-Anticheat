...
execute @s[type=player,tag=op,scores={gma=..0}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has allowed §agamemode 2§r to be used!"}]}

# deny
execute @s[type=player,tag=op,scores={gma=1..}] ~~~ scoreboard players set scythe:gamemode gma 0
execute @s[type=player,tag=op,scores={gma=1..}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has disallowed §4gamemode 2§r to be used!"}]}
