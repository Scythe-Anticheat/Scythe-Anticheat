...
execute @s[type=player,tag=op,scores={gms=..0}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has disallowed §4gamemode 0§r to be used!"}]}

# deny
execute @s[type=player,tag=op,scores={gms=1..}] ~~~ scoreboard players set scythe:config gms 0
execute @s[type=player,tag=op,scores={gms=1..}] ~~~ tellraw @a[tag=notify] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has allowed §agamemode 0§r to be used!"}]}
