...
registerCommand({
    ...
    execute: (message, args) => {
		...
		member.runCommandAsync("gamemode spectator");

		tellAllStaff(`§r§6[§aScythe§6]§r ${player.name} has changed ${player.id === member.id ? "their" : `${member.name}'s`} gamemode to spectator.`);
    }
});